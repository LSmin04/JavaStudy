...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    boolean isRemoved = members.remove("로제");
    System.out.println(isRemoved ? "삭제 성공" : "삭제 실패");

    System.out.println("삭제 후 남은 멤버: " + members);
    ...
    for(int i = 0, length = members.size(); i < length; i++){  // members.size()는 호출할 때마다 같은 값을 호출하고 있음. size()호출하는 것을 초기화 하는 곳에서 1번만 호출하도록 하면 된다.
